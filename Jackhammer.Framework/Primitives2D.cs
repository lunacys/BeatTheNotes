...
namespace Jackhammer.Framework
{
    /// <summary>
    /// Helper for drawing 2D primitives such as line, circle or rectangle. 
    /// </summary>
    public static class Primitives2D
    {
        ...
        public static void PutPixel(this SpriteBatch spriteBatch, Vector2 position, float brightness, Color color)
        {
            if (_pixel == null)
            {
                CreateThePixel(spriteBatch);
            ...
            spriteBatch.Draw(_pixel, position, new Color(color, brightness));
        }
        ...
        public static void DrawSmoothLine(this SpriteBatch spriteBatch, Vector2 startl, Vector2 endl, Color color)
        {
            float x0 = startl.X;
            float y0 = startl.Y;
            float x1 = endl.X;
            ...
            end.X = (float)Math.Round(x0);
            end.Y = y0 + gradient * (end.X - x0);

            float xgap = rfpart(x0 + 0.5f);

            Point pxl1 = new Point((int)end.X, (int)end.Y);

            if (steep)
            {
                PutPixel(spriteBatch, new Vector2(pxl1.X, pxl1.Y), rfpart(end.Y) * xgap, color);
                PutPixel(spriteBatch, new Vector2(pxl1.Y + 1, pxl1.X), fpart(end.Y) * xgap, color);
            }
            else
            {
                PutPixel(spriteBatch, new Vector2(pxl1.X, pxl1.Y), rfpart(end.Y) * xgap, color);
                PutPixel(spriteBatch, new Vector2(pxl1.X, pxl1.Y + 1), fpart(end.Y) * xgap, color);
            }
            float intery = end.Y + gradient;

            end.X = (float)Math.Round(x1);
            end.Y = y1 + gradient * (end.X - x1);
            xgap = fpart(x1 + 0.5f);

            Point pxl2 = new Point((int)end.X, (int)end.Y);
            if (steep)
            {
                PutPixel(spriteBatch, new Vector2(pxl2.Y, pxl2.X), rfpart(end.Y) * xgap, color);
                PutPixel(spriteBatch, new Vector2(pxl2.Y + 1, pxl2.X), fpart(end.Y) * xgap, color);
            }
            else
            {
                PutPixel(spriteBatch, new Vector2(pxl2.X, pxl2.Y), rfpart(end.Y) * xgap, color);
                PutPixel(spriteBatch, new Vector2(pxl2.X, pxl2.Y + 1), fpart(end.Y) * xgap, color);
            }

            for (int i = pxl1.X + 1; i < pxl2.X; i++)
            {
                if (steep)
                {
                    PutPixel(spriteBatch, new Vector2((int)intery, i), rfpart(intery), color);
                    PutPixel(spriteBatch, new Vector2((int)intery, i), fpart(intery), color);
                }
                else
                {
                    PutPixel(spriteBatch, new Vector2(i, (int)intery), rfpart(intery), color);
                    PutPixel(spriteBatch, new Vector2(i, (int)intery + 1), fpart(intery), color);
                }
                intery += gradient;
            }
        }
